...
\LMHash{}%
Any use of a type $T$ which is not well-bounded is a \Error{compile-time error}.
...
\item An expression of the form \code{$e$\,\,as\,\,$T$} is potentially constant
  ...
  \commentary{%
    It is a \Error{compile-time error} to evaluate the constant expression
...
\noindent
\code{%
$e$?.$m$<\List{A}{1}{r}>(\List{a}{1}{n},\ \PairList{x}{\!\!:\,\,a}{n+1}{n+k})}.
...
\noindent
\code{%
$v$.$m$<\List{A}{1}{r}>(\List{a}{1}{n},\ \PairList{x}{\!\!:\,\,a}{n+1}{n+k})}.
...
\LMHash{}%
It is a \Error{compile-time error} to invoke an instance method on a type literal
...
\LMHash{}%
A \Error{compile-time error} occurs if $D$ has the modifier \LATE.
...
\LMHash{}%
It is a \Error{compile-time error} if $T_i$, $1 \le i \le n$ is a deferred type.
...
[ONGOING] It is a warning to use a null aware operator (\code{?.},
\code{?[]}, \code{?..}, \code{??}, \code{??=}, or \code{...?}) on an
...
\Error{compile-time error} unless $D$ is a non-getter function declaration.  It is a
\Error{compile-time error} if $D$ declares more than two required positional
parameters, or if there are any required named parameters.  It is a
\Error{compile-time error} if $D$ declares at least one positional parameter,
