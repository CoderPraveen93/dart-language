...
\item If it returns without an object or it completes normally
  ...
  (\ref{null})
  and the current call to \code{moveNext()} on $j$ returns the \FALSE{} object,
...
\item If it throws an exception object $e$ and stack trace $t$
  ...
  Further calls to \code{moveNext()} must return the \FALSE{} object.
\end{itemize}
...
%% TODO(eernst), for review: The null safety spec uses 'shortening' and
%% 'shorting'; checking some dictionaries, it seems fine to use 'shorting'.
...
the \metaCode{TERM} combinator. \commentary{For example:}
...
\item
  ...
  \commentary{%
    ...
    there is no error.
    In this case the run-time value will be the null object,
    because we know that the variable is unassigned.%
  }
...
to the \TRUE{} object,
and \code{$e$\,\,\IS\,\,Object} evaluates to the \TRUE{} object wheneven
...
\LMHash{}%
This specification does not yet specify type inference in Dart.
...
A preliminary specification of the flow analysis is available at
