...
This can be achieved for any given program by making sure that
every top-level name reference is accessed using
an import prefix which is unique for the library that contains
the given declaration,
even when it is accessed from the same library.
This means that $C$ is \emph{always} accessed as, say,
\code{\metavar{library87381}.$C$}.%
}
