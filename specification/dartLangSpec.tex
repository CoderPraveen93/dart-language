...
\LMHash{}%
A compile-time error occurs if a static variable has a
...
It is a compile-time error if an optional formal parameter
...
\LMHash{}%
It is a compile-time error if a formal parameter has
...
\LMHash{}%
It is a compile-time error if an optional or named formal parameter has
the modifier \CONST{} or the modifier \LATE.
...
Otherwise, when the declared type is potentially non-nullable,
it is a compile-time error if an optional parameter has no default value.
It is a compile-time error if a required named parameter has a default value.
...
the identifier given in its function header.
...
that does not have an initializing expression.
A compile-time error occurs if $C$ has no generative constructors,
...
In this situation,
a compile-time error occurs if any of the following criteria holds:
...
is not a class building type
(\ref{classBuildingTypes}).
It is a compile-time error if two elements in said type list
...
It is a compile-time error if a mixin declaration declares
...
\LMHash{}%
It is a compile-time error for the mixin $M$ if this class declaration $N$
...
It is then a compile-time error if
...
\LMHash{}%
It is a compile-time error if
the \synt{forLoopParts} of a \synt{forElement}
declares an iteration variable that has
the modifier \CONST{} or the modifier \LATE,
using the same rules as with a \FOR{} statement
(\ref{for}).
...
It is a compile-time error unless $T$ is assignable to \code{Object}.
...
\LMHash{}%
A compile-time error occurs if the static type of $f$ is potentially nullable
and not \DYNAMIC{}
(\ref{typeNullability}).
...
It is a compile-time error unless both of the following are satisfied:
...
\LMHash{}%
Otherwise, $i$ is treated as

\noindent
\Let{$v$}{$e$}{$v$\,\,==\,\,\NULL\ ?\ \NULL\ :\ $v$.$m$<$A_1, \ldots,\ A_r$>(}

\noindent
\code{\qquad{}$a_1, \ldots,\ a_n,\ x_{n+1}$: $a_{n+1}, \ldots,\ x_{n+k}$: $a_{n+k}$)}

\noindent
which determines the static analysis and dynamic semantics of $i$.
\EndCase
...
\LMHash{}%
%% TODO(eernst): 'A compile-time error occurs if $e$ is a type literal.'
%% when we change the rules about type literals as receivers.
...
\LMHash{}%
Otherwise, $i$ is treated as
\Let{$v$}{$e$}{$v$\,\,==\,\,\NULL\ ?\ \NULL\ :\ $v$.\id},
which determines the static analysis and dynamic semantics.
\EndCase
...
\LMHash{}%
A compile-time error occurs if $D$ has the modifier \LATE.
A compile-time error occurs if the static type of $c$ is not
...
A compile-time error occurs if $D$ contains
the modifier \CONST{} or the modifier \LATE.
...
\commentary{%
With a receiver whose static type is \code{Never},
...
It is not an error to apply an expression of type \code{Never}
in the function position of a function call.
Implementations that provide feedback about dead or unreachable code
are encouraged to indicate that any arguments to the call
will not be evaluated at run time.%
}
...
It is an error for the initializer expression of a \LATE{} local
variable to use a prefix \AWAIT{} expression that is not nested inside
...
[ONGOING] It is a warning to use a null aware operator (\code{?.}, \code{?[]}, \code{?..}, \code{??}, \code{??=}, or
...
The static type of an expression \code{$e$!} is \NonNullType{$T$} where $T$ is the
static type of $e$.
